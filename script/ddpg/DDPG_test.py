...
for i in range(max_episodes+1):
    ...
    if (i%100)==0:
        epi_data.append(i)
        reward_data.append( np.mean(avg_return_list))
        plt.plot(epi_data, reward_data, 'r')
        plt.draw()
        plt.pause(0.0001)
        plt.clf()

    if (i%100)==0:
        print('[{}/{}] avg_return_list : {:.3f}, actor_noise.x_prev) : {:.3f}'.format(i,max_episodes, np.mean(avg_return_list), np.mean(actor_noise.x_prev)))
    
    if (np.mean(avg_return_list) > 500): # Threshold return to success cartpole
        print('[{}/{}] avg_return_list : {:.3f}, actor_noise.x_prev) : {:.3f}'.format(i,max_episodes, np.mean(avg_return_list), np.mean(actor_noise.x_prev)))
